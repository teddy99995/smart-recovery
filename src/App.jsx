...
const generateAllSlots = () => {
  const slots = [];
  for (let h = 10; h < 22; h++) {
    slots.push(`${h}:00-${h}:30`); slots.push(`${h}:30-${h + 1}:00`);
  }
  return slots;
};
      ...
      <div className="max-w-7xl mx-auto space-y-6 w-full flex-1">
        ...
        {!currentUser ? (
          ...
          <div className="bg-white/95 rounded-3xl shadow-xl flex flex-col min-h-[700px] overflow-hidden">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-white font-bold flex items-center gap-2">
                    {currentUser.name}
                    <button onClick={() => setShowPwdModal(true)} className="text-[11px] bg-white/10 hover:bg-white/20 px-2 py-1 rounded-md text-white/70 transition-colors flex items-center gap-1 font-normal ml-1 border border-white/10"><Key size={10} /> 修改密碼</button>
                  </h2>
            ...
            <button onClick={exportToGoogleSheets} className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-2 px-4 rounded-xl text-sm flex items-center gap-2 m-4 w-fit">
